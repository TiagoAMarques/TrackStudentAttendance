...
export async function getDashboard(user: ChatGPTUser): Promise<DashboardData> {
  ...
  const attendanceTrend=await db.prepare(`SELECT sc.id,sc.class_id AS classId,sc.class_date AS date,COUNT(DISTINCT a.student_id) AS present FROM scheduled_classes sc LEFT JOIN class_sessions cs ON cs.course_id=sc.course_id AND cs.title=sc.class_id LEFT JOIN attendance a ON a.session_id=cs.id AND a.voided_at IS NULL WHERE sc.course_id=? AND sc.session_id IS NOT NULL GROUP BY sc.id ORDER BY sc.class_date,sc.class_time,sc.class_id`).bind(course.id).all<{id:string;classId:string;date:string;present:number}>();
  return {
